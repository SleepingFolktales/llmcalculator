...
def estimate_memory_gb(
    ...
    if is_moe and active_params_b:
        model_weights = active_params_b * quant_bpp(quant)
    else:
        model_weights = params_b * quant_bpp(quant)
...
def best_quant_for_budget(
    ...
    for quant in QUANT_HIERARCHY:
        mem_gb = estimate_memory_gb(params_b, quant, context_tokens, is_moe, active_params_b)
        if mem_gb <= budget_gb:
            return (quant, mem_gb)
    ...
    if context_tokens > 2048:
        ...
        for quant in QUANT_HIERARCHY:
            mem_gb = estimate_memory_gb(params_b, quant, half_context, is_moe, active_params_b)
            if mem_gb <= budget_gb:
                return (quant, mem_gb)
...
def scenario_memory_gb(
    ...
    single_instance_mem = estimate_memory_gb(
        params_b, quant, context_tokens, is_moe, active_params_b
    )
    
    model_weights = (active_params_b if is_moe and active_params_b else params_b) * quant_bpp(quant)
